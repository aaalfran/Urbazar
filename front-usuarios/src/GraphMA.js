export class GraphMA{
    ...
    addCapacity(){
        let tmp = [...Array(this.capacity + (this.capacity/2) * 3)]
    ...
    removeEdge(element1,element2){
        ...
        let start = this.vertexes.indexOf(element1)
        let end = this.vertexes.indexOf(element2);
        ...
        for (let i = 0; i < this.capacity; i++) {
            for (let k = 0; k < this.capacity; k++){
                if(this.matrix[i][k] !== undefined){
                    let s = "(" + this.vertexes[i] + "," + this.vertexes[k]+ ")";
                    tuplas = tuplas + s + ";";
                }
            }
        }
        let a= tuplas.slice(-1,1);
        tuplas= tuplas + "]"
        total = total + tuplas
        return total;
    }
