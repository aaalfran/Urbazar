...
class RegisterPC extends Component {
  ...
  validarTelefono() {
    ...
    if (
      !(
        expresionRegular1.test(numeroTelefono) ||
        expresionRegular2.test(numeroTelefono)
      )
    ) {
  ...
  cargarUsernames() {
    axios
      .get(`${data.url}/personas?filter[fields][username]=true`)
  ...
  async signUp() {
    ...
    const identificacionV = document.getElementById('identificacion').value
    const generoV = document.getElementById('genero').value
    ...
    const data = {
      id: parseInt(idV, 10),
      identificacion: identificacionV,
      id_etapa: 0,
      nombre: nombreV,
      telefono: telefonoV,
      correo: correoV,
